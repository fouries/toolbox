...
assert.match(backendMain, /ContentService\.get_daily_brief/, 'daily brief endpoint should call content service')
assert.match(backendMain, /@app\.get\("\/api\/hot-search"/, 'backend should expose /api/hot-search')
assert.match(backendMain, /ContentService\.get_hot_search/, 'hot search endpoint should call content service')
assert.match(backendMain, /@app\.get\("\/api\/hot-search\/detail"/, 'backend should expose /api/hot-search/detail')
assert.match(backendMain, /ContentService\.get_hot_search_detail/, 'hot search detail endpoint should call content service')
...
assert.match(hotSearchDetailPage, /查看百度原视频/, 'baidu videos should expose original video link action when available')
assert.match(hotSearchDetailPage, /百度视频按需代理播放，不下载保存/, 'baidu videos should explain proxy-only playback')
assert.match(backendTianApi, /baidu_video_proxy_only_v1/, 'baidu rich-media cache should be bumped after proxy-only video normalization')
assert.match(backendTianApi, /_proxied_baidu_image_url\(poster\)/, 'baidu video posters should use the image proxy')
