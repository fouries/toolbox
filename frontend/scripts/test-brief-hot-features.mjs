...
assert.match(hotSearchPage, /<image\s+class="hot-image"[\s\S]*v-if="shouldShowHotImage\(item\)"[\s\S]*:src="item\.image"[\s\S]*mode="aspectFill"/, 'baidu hot search items should render official thumbnail images when present')
assert.match(hotSearchPage, /\.hot-image\s*\{[\s\S]*width:\s*132rpx;[\s\S]*height:\s*92rpx;[\s\S]*object-fit:\s*cover;/, 'hot search thumbnail should use compact fixed dimensions')
