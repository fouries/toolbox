...
assert.match(hotSearchDetailPage, /<text class="keyword-title">\{\{ detail\?\.keyword \|\| keyword \|\| '未知热搜' \}\}<\/text>[\s\S]*<view\s+class="hot-image-section"\s+v-if="hotImage"[\s\S]*<image\s+class="hot-image"\s+:src="hotImage"\s+mode="widthFix"[\s\S]*<text\s+class="keyword-desc"\s+v-if="detail\?\.summary">\{\{ detail\.summary \}\}<\/text>/, 'hot search detail page should render only keyword, image, and summary in order')
assert.doesNotMatch(hotSearchDetailPage, /detailContentLines|detail-content-inline|相关资讯|relatedNews|openNewsDetail|news-list|news-card|note-card|keyword-meta|action-row|copyHotLink/, 'hot search detail page should remove extra related-news/content/copy sections')
