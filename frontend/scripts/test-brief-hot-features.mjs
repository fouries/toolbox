...
assert.match(hotSearchDetailPage, /<view\s+class="detail-content-inline"\s+v-if="detailContentLines\.length"[\s\S]*v-for="line in detailContentLines"[\s\S]*\{\{ line \}\}/, 'hot search detail should render backend hot-search content inside the first keyword card')
assert.match(hotSearchDetailPage, /detailContentLines\s*=\s*computed\(\(\)\s*=>[\s\S]*detail\.value\?\.sections[\s\S]*filter\(section => section\.title !== `\$\{platformName\.value\}返回信息`\)[\s\S]*flatMap/, 'inline hot-search content should reuse backend sections but hide platform returned-info heading')
assert.match(hotSearchDetailPage, /detailContentLines[\s\S]*detail\.value\?\.summary/, 'inline hot-search content should fall back to backend summary')
assert.match(hotSearchDetailPage, /filter\(line => line !== detail\.value\?\.summary\)/, 'inline hot-search content should avoid duplicating the summary already shown above')
...
assert.match(hotSearchDetailPage, /\.detail-content-inline\s*\{[\s\S]*margin-top:\s*18rpx;/, 'inline hot-search content should live in the first keyword card')
assert.match(hotSearchDetailPage, /getHotSearchDetail/, 'hot search detail page should fetch backend detail content')
