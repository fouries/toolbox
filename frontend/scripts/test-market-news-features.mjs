...
assert.match(api, /image\?:\s*string[\s\S]*originalImage\?:\s*string/, 'NewsDetail should expose original article image fields')
assert.match(api, /getNewsDetail\s*=\s*\(url:\s*string,\s*localUrl\?:\s*string,\s*image\?:\s*string\)/, 'api should expose getNewsDetail(url, localUrl, image)')
assert.match(api, /\/api\/news\/detail\?url=\$\{encodeURIComponent\(url\)\}\$\{image \? `&image=\$\{encodeURIComponent\(image\)\}` : ''\}/, 'getNewsDetail should pass preferred list image to backend')
...
assert.match(infoNewsPage, /\/pages\/news-detail\/index\?url=\$\{encodeURIComponent\(safeUrl\)\}&image=\$\{encodeURIComponent\(item\.picUrl \|\| ''\)\}/, 'info news cards should navigate to native detail page with list image')
...
assert.match(backendMain, /@app\.get\("\/api\/news\/image-proxy"/, 'backend should expose article image proxy endpoint')
