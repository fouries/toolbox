...
assert.match(source, /\.quick-tool-card\s*\{[\s\S]*min-height:\s*76rpx;/, 'popular tool cards should use a shorter compact card height')
assert.match(source, /\.quick-tool-card\s*\{[\s\S]*padding:\s*10rpx 10rpx;/, 'popular tool cards should use tighter padding')
...
assert.match(source, /@media\s*\(min-width:\s*768px\)[\s\S]*\.quick-tool-card\s*\{[\s\S]*min-height:\s*52px;/, 'desktop popular tool cards should be shorter')
